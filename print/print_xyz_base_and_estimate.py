...
y_data = myfunction.load_pickle(r"C:\Users\WRS\Desktop\Matsuyama\laerningdataandresult\Robomech_GRU\allusenan\real20250821_162546.pickle")
estimation_array= myfunction.load_pickle(r"C:\Users\WRS\Desktop\Matsuyama\laerningdataandresult\Robomech_GRU\allusenan\result20250821_162546.pickle")

list_estimation_array = [t.cpu().tolist()[0] for t in estimation_array]

half = len(y_data) // 2

x  = [row[9]  for row in y_data[half::1]]
y  = [row[10] for row in y_data[half::1]]
z  = [row[11] for row in y_data[half::1]]

ex = [row[9]  for row in list_estimation_array[half::1]]
ey = [row[10] for row in list_estimation_array[half::1]]
ez = [row[11] for row in list_estimation_array[half::1]]

nx  = [row[9]  for row in y_data[:half:1]]
ny  = [row[10] for row in y_data[:half:1]]
nz  = [row[11] for row in y_data[:half:1]]

enx = [row[9]  for row in list_estimation_array[:half:1]]
eny = [row[10] for row in list_estimation_array[:half:1]]
enz = [row[11] for row in list_estimation_array[:half:1]]

...
class FingerDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, future_estimate_horizon, obs_horizon, now_estimate_horizon, use_data, mode):
        train_split=0.7
        ...
        indices = create_sample_indices(episode_ends = episode_ends,sequence_length=future_estimate_horizon,
                                                pad_before=obs_horizon-1, pad_after=now_estimate_horizon-1)
    ...
    def __getitem__(self, idx):
        ...
        nsample = sample_sequence(
            ...
            sequence_length=self.future_estimate_horizon,
        ...
        nsample['condition'] = nsample['condition'][:self.obs_horizon,:]
        return nsample
    
def main():
    future_estimate_horizon = 1
    obs_horizon = 32
    ...
    train_data = FingerDataset(dataset_path= data_path, future_estimate_horizon=1, obs_horizon=32, now_estimate_horizon=1,use_data=usedata, mode = "train")
    test_data = FingerDataset(dataset_path= data_path, future_estimate_horizon=1, obs_horizon=32, now_estimate_horizon=1,use_data=usedata, mode = "test")

...
def train(selector, train_loader, optimizer, class_weight):
    ...
    for mag_seq,mu_pair, label ,w in train_loader:
...
def main():
    ...
    train_ds = torch.utils.data.TensorDataset(mag_seq_tr.cpu(),mu_pair_tr.cpu(), label_tr.cpu().long(), m_w_tr.cpu())
    test_ds  = torch.utils.data.TensorDataset(mag_seq_te.cpu(),mu_pair_te.cpu(), label_te.cpu().long())
    ...
    selector = MyModel.SelectorGRU(mag_dim=9, hidden=64, num_layers=1, fc_dim=64).to(device)

...
#1エポックの学習
def train(model, data_loader, optimizer, criterion, scalerforAMP):
    model.train()
    loss_mean = 0
    ...
    for j, (x_train_data, y_train_data) in enumerate(data_loader):
        ...
        scalerforAMP.scale(loss).backward()


        # loss.backward()
        # with profiler.record_function("optimize"):
        scalerforAMP.step(optimizer)
        # optimizer.step()
        scalerforAMP.update()   
...
#---------------------------------------------------------------------------------- --------------------------------------
    ...
    result_dir = r"C:\Users\WRS\Desktop\Matsuyama\laerningdataandresult\Robomech_GRU\batch128amp"
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            train_loss = train(model, train_loader, optimizer, criterion, scalerforAMP)
